...
public:
    ...
    void evaluate(int level, std::chrono::steady_clock::time_point begin, std::string last_processed_var = "")
    {
        ...
        if (level > 0)
        {
            ...
            level_candidate_var_umap[level] = varname; // TODO: NO DEBERIA IR ESTO DENTRO DEL IF DE CANDIDATE_VARS, asi siempre estan sincronizadas las estructuras.
        }
        else
        {
        ...
        else
        {
            ...
            while (search)
            {
                ...
                cout << "level : " << level << " variable : " << varname << " seeked value : " << seek_value << std::endl;
                if (seek_value != 0)
                {
            ...
            for (Iterator *triple_iterator : this->query_iterators[var_to_delete_iters])
            {
                ...
                if (triple_iterator->created_by == var_to_delete_iters)
                {
                    // Entering here means that the iterator was created by 'var_to_delete_iters'. Therefore it has to be first removed from the iterators vector of 'var_to_delete_iters' and 'rel_var', and then deleted.
                    {
                        ...
                        std::vector<Iterator *>::iterator it = std::find_if(v.begin(), v.end(), [&iter_id](auto it)
                                                                            { return it->get_id() == iter_id; });
                        if (it != v.end())
                        {
                            v.erase(it);
                        }
    ...
    void shrink_candidate_stack()
    {
        ...
        std::unordered_map<int, std::string>::iterator it = std::find_if(level_candidate_var_umap.begin(), level_candidate_var_umap.end(), [&var_to_erase](auto it)
                                                                         { return it.second == var_to_erase; });
    ...
    std::string get_next_variable(int level, std::string last_processed_var = "")
    {
        ...
        if (level_candidate_var_umap.size() == 0)
        {
            ...
            sort((varmin_pairs).begin(), (varmin_pairs).end(), [&](pair<string, int> a, pair<string, int> b) -> bool
    ...
    {
        if (std::find((this->single_vars).begin(), (this->single_vars).end(), var) == (this->single_vars).end())
        {
            return false;
        }
        return true;
    ...
    */
    ...
    {
        if (std::find_if(level_candidate_var_umap.begin(), level_candidate_var_umap.end(), [&var](auto it)
                         { return it.second == var; }) == level_candidate_var_umap.end())
        {
            return false;
        }
        return true;
